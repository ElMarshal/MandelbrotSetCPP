...
int main()
{
	...
	for (size_t y = 0; y < BUFFER_HEIGHT; ++y)
	{
		for (size_t x = 0; x < BUFFER_WIDTH; ++x)
		{
			...
			accumelator.r /= SAMPLE_COUNT;
			accumelator.g /= SAMPLE_COUNT;
			accumelator.b /= SAMPLE_COUNT;
			accumelator.a /= SAMPLE_COUNT;
			color_buffer[y*BUFFER_WIDTH + x] = ColorRGBA8{ (uint8_t)accumelator.r, (uint8_t)accumelator.g, (uint8_t)accumelator.b, (uint8_t)accumelator.a };
		}
